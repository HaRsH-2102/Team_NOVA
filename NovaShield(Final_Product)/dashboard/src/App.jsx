...
const PIE_COLORS = ['#1b998b', '#ff9f1c', '#ef476f', '#118ab2', '#735d78'];
...
function App() {
  ...
  useEffect(() => {
    ...
    loadSnapshot().catch(() => setConnectionState('error'));
    connect();
  ...
  return (
    <div className="shell">
      <header className="hero">
        <div>
          <p className="eyebrow">Reactive Security Control Plane</p>
          <h1>Nova Shield Command Center</h1>
          <p className="lead">
            Live gateway telemetry, security events, request logs, and streaming
            performance signals over WebSocket.
          </p>
        </div>
      ...
      <section className="card-grid">
        ...
        <MetricCard title="Blocked" value={counters.blocked_requests} accent="bad" />
        <MetricCard
          title="Rate Limited"
...
function applyLiveEvent(current, event) {
  ...
  switch (event.event) {
    ...
    case 'security_event':
      return {
        ...fallback,
        blacklisted_ips: mergeBlacklistedIp(fallback.blacklisted_ips, event.entry),
