...
const totalRequests = Number(process.env.TOTAL_REQUESTS || 5000);
  ...
  if (index < 3800) {
    return {
      ...
      options: {
        method: "GET",
        headers: {
          [demoHeader]: `10.2.${Math.floor((index - 2600) / 60) + 1}.${((index - 2600) % 60) + 1}`,
        },
      },
    };
  }
    ...
    return {
      ...
      options: {
        ...
        headers: {
          ...
          [demoHeader]: `10.4.${Math.floor((index - 3800) / 40) + 1}.25`,
        },
        body: JSON.stringify({
          username: "burst-user",
          password: `pw-${index}`,
  ...
  if (index < 4800) {
    return {
      ...
      options: {
        ...
        headers: {
          "content-type": "application/json",
          [demoHeader]: `10.6.${Math.floor((index - 4600) / 20) + 1}.${((index - 4600) % 20) + 1}`,
        },
        body: JSON.stringify({
...
console.log(`Dispatching ${totalRequests} requests to ${baseUrl} all at once...`);
