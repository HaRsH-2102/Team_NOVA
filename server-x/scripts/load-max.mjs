...
// The "max possible" load default set to 50,000 concurrent sockets
const totalRequests = Number(process.env.TOTAL_REQUESTS || 50000);
  ...
  if (index < 38000) {
    return {
      ...
      options: {
        method: "GET",
        headers: {
          [demoHeader]: `10.2.${Math.floor((index - 26000) / 60) + 1}.${((index - 26000) % 60) + 1}`,
        },
      },
    };
  }
    ...
    return {
      ...
      options: {
        ...
        headers: {
          ...
          [demoHeader]: `10.4.${Math.floor((index - 38000) / 40) + 1}.25`,
        },
        body: JSON.stringify({
          username: "burst-user",
          password: `pw-${index}`,
  ...
  if (index < 48000) {
    return {
      ...
      options: {
        ...
        headers: {
          "content-type": "application/json",
          [demoHeader]: `10.6.${Math.floor((index - 46000) / 20) + 1}.${((index - 46000) % 20) + 1}`,
        },
        body: JSON.stringify({
...
console.log(`Warning: 50,000 requests will take some time, but throttling prevents dropped connections.`);
